...
static const char *kinectNames[NKINECT] ={"kinect_front_color","kinect_behind_color"};//kinect的名字 
...
int main(int argc, char **argv)
{
	...
	//创建图像话题订阅者
	ros::Subscriber forntImgSub=n->subscribe("/supermarketRobot/kinect_front_color/image",10,FrontImgCallBack);
	ros::Subscriber behindImgSub=n->subscribe("/supermarketRobot/kinect_behind_color/image",10,BehindImgCallBack);
...
bool Init(int argc, char **argv)//初始化函数
{
	...
	if(InitKinect()==true)//kinect初始化
		ROS_INFO("InitKinect succeed!");
	else
	{
		ROS_ERROR("InitKinect failed!");
		return false;
	}
	return true;
}
...
bool InitKinect()
{
	ros::ServiceClient kinectEnableClient;   
	webots_ros::set_int kinectEnableSrv;
	
	for(int i=0;i<NKINECT;i++)
	{
		//请求服务 superMarketRobot/kinect_xxx_color/enable
		kinectEnableClient = n->serviceClient<webots_ros::set_int>(ROS_NODE_NAME+std::string("/")+ kinectNames[i]
															+ std::string("/enable"));   
		kinectEnableSrv.request.value = 64;
		if (kinectEnableClient.call(kinectEnableSrv) &&kinectEnableSrv.response.success)
			ROS_INFO("Open kinect succeed.");
		else
		{     
			ROS_ERROR("Failed to open kinect");
			return false;
		}
	} 
	return true;   
